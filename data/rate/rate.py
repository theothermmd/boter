...
with tqdm( 
    ...
    for i in range(1 , 200) :
        for attempt in range(3):
            try :
                response = requests.get(f'https://vod.tarinet.ir/wp-json/wp/v2/rate?per_page=100&page={i}' , timeout=30)
                break
            except :
                continue
        ...
        if response.status_code == 200 :
            ...
            for j in response.json() :
                rate['rate'].append({'name' : j['name'] , 'id' : j['id']})
...
with open('rate.json', 'w', encoding='UTF-8') as file:
    file.write(json.dumps(rate, ensure_ascii=False))
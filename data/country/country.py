...
with tqdm( 
    ...
    for i in range(1 , 200) :
        for attempt in range(3):
            try :
                response = requests.get(f'https://www.vod.tarinet.ir.cartoonflix.ir/wp-json/wp/v2/country?per_page=100&page={i}' , timeout=30)
                break
            except :
                continue
        ...
        if response.status_code == 200 :
            ...
            for j in response.json() :
                country['country'].append({'name' : j['name'] , 'id' : j['id']})
...
with open('country.json', 'w', encoding='UTF-8') as file:
    file.write(json.dumps(country, ensure_ascii=False))